...
spec = importlib.util.spec_from_file_location("reforms", "src/uk_budget_data/reforms.py")
...
if slr_reform:
    ...
    print(f"{'Year':<8} {'Model Cost':>13} {'OBR Cost':>13} {'Difference':>13} {'Fewer Payers':>18}")
    print("=" * 72)

    for year in [2025, 2026, 2027, 2028, 2029]:
        # Calculate student loan repayments (modelled)
        baseline_slr = baseline.calculate("student_loan_repayments_modelled", period=year).sum() / 1e9
        reform_slr = reformed.calculate("student_loan_repayments_modelled", period=year).sum() / 1e9
        ...
        baseline_repayers = (baseline.calculate("student_loan_repayments_modelled", period=year) > 0).sum()
        reform_repayers = (reformed.calculate("student_loan_repayments_modelled", period=year) > 0).sum()
        ...
        print(f"{year:<8} £{cost:>11.3f}bn £{obr_value:>11.3f}bn £{difference:>11.3f}bn {fewer_payers:>16,.0f}")
    ...
    print(f"{'Year':<8} {'Baseline HBAI':>15} {'Reform HBAI':>15} {'HH Gain':>15}")
    print("=" * 58)

    for year in [2026, 2027, 2028, 2029]:
        baseline_hbai = baseline.calculate("hbai_household_net_income", period=year, map_to="household").sum() / 1e9
        reform_hbai = reformed.calculate("hbai_household_net_income", period=year, map_to="household").sum() / 1e9
        # Household gain = higher income in reform (fewer repayments)
        hh_gain = reform_hbai - baseline_hbai

        print(f"{year:<8} £{baseline_hbai:>13.2f}bn £{reform_hbai:>13.2f}bn £{hh_gain:>13.3f}bn")
